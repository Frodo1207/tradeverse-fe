...
import { Swords, Trophy, Globe, Search, User, Star, Gift, ArrowRight, Users, Target, Crown, Flame, Timer, ChevronRight } from 'lucide-react';
...
const CompetitiveView = ({ onBack }) => {
                    ...
                    for (const g of response.games || []) {
                        ...
                        try {
                            if (g.category) {
                                ...
                                type = String((meta.type || 'single')).toLowerCase();
                            }
                        } catch { void 0; }
                        ...
                        mapped.push({
                            ...
                            players: '1K+',
                            progress: 0,
                            image: g.iconUrl || 'https://images.unsplash.com/photo-1542751371-adc38448a05e?q=80&w=2070&auto=format&fit=crop',
                            metadata: g.metadata || '',
                            url: g.url
                        });
                    }
    ...
    return (
        <div className="relative w-full min-h-screen pt-24 pb-12 px-4 md:px-8 overflow-y-auto animate-fade-in max-w-7xl mx-auto">
                ...
                <div className="absolute inset-0 z-20 p-8 md:p-12 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-start">
                        <div className="flex items-center gap-2 text-xs md:text-sm font-mono bg-black/50 backdrop-blur-md px-4 py-2 rounded-full border" style={{ borderColor: `${config.color}50`, color: config.color }}>
                            <Globe size={14} /> ONLINE • COMPETITIVE ZONE
                        </div>
                    ...
                    <div className="relative">
                        ...
                        <div className="flex flex-col md:flex-row items-end justify-between gap-8 mt-8">
                            ...
                            <div className="w-full md:w-2/3 bg-black/40 backdrop-blur-sm rounded-xl p-4 border border-white/5">
                                <LevelProgressBar gameMode="Competitive" theme="red" variant="banner" />
                            </div>
                            ...
                            <button className="w-full md:w-auto bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white px-6 py-4 rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 transition-all hover:scale-105 whitespace-nowrap h-fit">
                                <Star size={16} className="text-yellow-400" /> My Stats
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="mb-16">
                <LevelRewardsTrack currentLevel={progress['Competitive'].level} theme="red" />
            </div>
            ...
            <div>
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 border-b border-white/10 pb-6">
                    <div className="flex gap-8 overflow-x-auto">
                        <button className="text-white font-bold text-2xl border-b-2 pb-6 -mb-6.5 whitespace-nowrap" style={{ borderColor: config.color }}>Active Leagues</button>
                        <button className="text-gray-500 hover:text-white font-bold text-2xl transition-colors pb-6 whitespace-nowrap">Tournaments</button>
                        <button className="text-gray-500 hover:text-white font-bold text-2xl transition-colors pb-6 whitespace-nowrap">Scrims</button>
                    </div>

                    <div className="flex items-center gap-3 text-xs font-mono text-gray-400 mt-6 md:mt-0">
                        SORT BY: <span className="text-white font-bold cursor-pointer bg-white/10 px-2 py-1 rounded hover:bg-white/20 transition-colors">PRIZE POOL</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {loading ? (
                        ...
                        <div className="col-span-full text-center py-20 text-gray-500">No competitive games available yet.</div>
                    )}
                </div>
            </div>
